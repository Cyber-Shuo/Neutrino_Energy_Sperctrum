...
# define global variant neutrino energy /Mev, baseline /km, and 4 situation
neutrino_energy  = []
F_L_E_1, F_L_E_2, F_L_E_3, F_L_E_4 = [],[],[],[]
L_division_E = np.arange(0, 50, 0.3)
baseline = 58

# define cross section function /km^2
def sigma_E(k):
    E_e = k - (939.565-938)
...
# define quality squared difference /ev^2
...
# get e
for i in L_division_E:
    e = baseline/i
    neutrino_energy.append(e)

# get 4 situation 
for i, j in zip(L_division_E, neutrino_energy):
    F_L_E_1_vv = P_ee_none*phi(j)*sigma_E(j)
    F_L_E_1.append(F_L_E_1_vv)

    F_L_E_2_vv = P_ee_only21(i)*phi(j)*sigma_E(j)
    F_L_E_2.append(F_L_E_2_vv)

    F_L_E_3_vv = P_ee_NO(i)*phi(j)*sigma_E(j)
    F_L_E_3.append(F_L_E_3_vv)

    F_L_E_4_vv = P_ee_IO(i)*phi(j)*sigma_E(j)
    F_L_E_4.append(F_L_E_4_vv)

fig, ax = plt.subplots()
ax.set_xlim([0, 10])
ax.set_ylim([0, 2*10**(-43)])
ax.plot(neutrino_energy, F_L_E_1, linestyle = '-', color = 'black', label = 'NO Oscillation')
ax.plot(neutrino_energy, F_L_E_2, linestyle = '--', color = 'b', label = '1-'+r'$\rm{P_{21}}$'+'Oscillation')
ax.plot(neutrino_energy, F_L_E_3, linestyle = '-.', color = 'g', label = 'NO')
ax.plot(neutrino_energy, F_L_E_4, linestyle = ':', color = 'r', label = 'IO')
